...
$csvs = Get-ClusterSharedVolume -Cluster $ClusterName | where name  -NotLike "*Collect*"
...
foreach ($node in $nodes){
   ...
   foreach($csv in $csvs)
   {
      ...
      [string]$CSVPath = $csv.SharedVolumeInfo.FriendlyVolumeName
      $destfile = "`"" + $CSVPath + "\" + $file + $number + ".vhdx" + "`" "
      $targets = $targets + $destfile
      $testfile = Get-ChildItem -Path $destfile -ErrorAction SilentlyContinue

      if ($testfile -ne $null -and $testfile.Length -eq $filesizebytes)
      {
            $c = $null
      }
      else { $c = " -c" + $filesize}
